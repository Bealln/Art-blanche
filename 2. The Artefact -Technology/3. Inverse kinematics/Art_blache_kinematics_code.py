...
arduino = serial.Serial('/dev/tty.usbmodem14601', 9600, timeout = 1)

def inverse_kinematics(cx,cy):   
    """""
    INVERSE KINEMATICS 
    ** Something is not perfectly right, see "inverse_kinematics_calculations_test.py" for testing
    PARAMETERS
    
    
    """

    #Defining constant and starting positions 

    stepperA_resolution= 1.8  #steps = 200 (Nema23)
    stepperB_resolution = 0.9  #steps = 400 (Nema17)
        #print("Starting angles", arm.angles) #This is always 0,0
        #print("Starting end effector possition", arm.ee) #This is always (65,0,0)
    
    print ("position from camera", cx, cy)
    #Resolution changes the Mac resolution is 1280 x 720  and an A· is 420x297 mm
    x = cx/3
    y = cy/2.4
    ...
    arm.ee = [x, y, 0]
    print("Desired new position", arm.ee)
    ...
    #The angles are calculates in radians and then are passed to degrees
    #print("Angles in radian", arm.angles) #This prints the angles in radians
    angles_in_degrees = np.round(np.rad2deg(arm.angles))
    print("Angles in degrees", angles_in_degrees) #This prints the angles in degrees
    
    stepperA_angle = angles_in_degrees[0]
    stepperB_angle = angles_in_degrees[1]
    
    print("New angles",stepperA_angle,stepperB_angle)
        
    stepsA = stepperA_angle/stepperA_resolution
    stepsB= stepperB_angle/stepperB_resolution
    

    print("Steps for motor", stepsA,stepsB)
    
    
    #time.sleep(2)
   # while 1:
    message = '<%.1f,%.1f>'  %(stepsA, stepsB)
    #message = '<+20,+25>'
    #message = '<%s,%s>' %(stepsA, stepsB)
    print(message)
...
while(1):
    ...
    ##### Show final image ########
    #I commented this so it would run faster (It does not show the analysis)
    ...
    inverse_kinematics(cx=cx, cy=cy)
    time.sleep(1.5)
